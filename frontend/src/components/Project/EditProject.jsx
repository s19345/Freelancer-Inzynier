  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || "B??d aktualizacji projektu");
      }

      const updatedProject = await response.json();
      setSubmitStatus("Projekt zaktualizowany pomy?lnie!");

      navigate(`/projects/${projectId}`);

    } catch (err) {
      setSubmitStatus(`B??d: ${err.message}`);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="max-w-3xl mx-auto bg-white p-6 rounded shadow">
      ...
      <label className="block mb-2">
        ...
        >
          ...
          <option value="completed">Uko?czony</option>
      ...
      <label className="block mb-4">
        Wsp?pracownicy (oddziel przecinkami)
        <input
          type="text"
          name="collaborators"
      ...
      {submitStatus && (
        <p className={`mt-4 ${submitStatus.startsWith("B??d") ? "text-red-600" : "text-green-600"}`}>
          {submitStatus}
        </p>
      )}
    </form>
